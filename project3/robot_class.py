...
class robot:
    ...
    def sense(self):
        ...
        # compute the distances between the robot for each landmarks
        for indx in range(self.num_landmarks):
        
            # compute dx and dy distances between the robot and the landmark[i]
            dx, dy = np.array(self.landmarks[indx]) - np.array([self.x, self.y])  
            # account for measurement noise by *adding* a noise component to dx and dy
            dx += self.rand() * self.measurement_noise
            dy += self.rand() * self.measurement_noise
            
            # if either of the distances, dx or dy, fall outside of the internal var, then we cannot record them
            if np.sqrt( dx**2 + dy**2 ) < self.measurement_range:
                measurements.append([indx, dx, dy])
